...
As transformer models increasingly became the default way to approach AI, the documentation expanded significantly to include new models and new usage patterns. But the new content was added incrementally without really considering how the audience and transformer landscape have evolved.

I think this is why the documentation experience (DocX) feels disjointed, difficult to navigate, and outdated. Basically, a mess.
...
With the redesign, the Transformers documentation will be more code-first and solution-oriented. The redesign will also feature a more *complete* and beginner-friendly onboarding experience that bridges code and concept.

Once developers have a basic understanding, they can progressively level up their understanding of the library.

## Toward a more organic structure [#organic-structure]
...
Rigidity prevented naturally occuring content structures from emerging and prevented the documentation from adapting and scaling. Documentation about one topic soon spanned several sections, because it was what the structure dictated, and not because it made sense.

It's okay if the structure is complex, but it's not okay if it's complex *and* not easy to find your way around.

The redesign will replace rigidity for flexibility to enable the documentation to grow and evolve.

## Integration versus amendment [#integration-versus-amendment]

Tree rings provide a record of the climate in the past (drought, flood, wildfire, etc.). In a way, the Transformers documentation also has its own tree rings or *eras* that capture the evolution of the transformer landscape:

1. **Not just text** era: Transformer models are used across modalities like [computer vision](https://hf.co/docs/transformers/tasks/image_classification), [audio](https://hf.co/docs/transformers/tasks/asr), [multimodal](https://hf.co/docs/transformers/tasks/text-to-speech), and not just text.
...
Each era *incrementally* added new content to the documentation, unbalancing and obscuring previous parts of the documentation. Content is sprawled over a greater surface, navigation is more complex.

<div class="flex justify-center">
  <img class="rounded-sm" src="https://huggingface.co/datasets/stevhliu/personal-blog/resolve/main/transformer.png"/>
</div>
<p class="text-xs">In the tree ring model, new content is layered progressively over the previous material. Whereas in the integrated model, content coexists together as a part of the overall documentation.</p>

A redesign will help rebalance the overall documentation experience. Content will feel native and integrated rather than bolted on.

The next post will document the redesign process.
