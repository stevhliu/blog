...
<FloatingTOC
  items={[
    ...
    { text: "buckets", href: "#buckets", level: 3 },
...
[Buckets](https://huggingface.co/docs/huggingface_hub/en/guides/buckets) are a type of storage container designed for storing intermediate stuff, like checkpoints from a training run or processed data from a data pipeline. These files can be overwritten, deleted, and don't require versioning. When you're ready, publish these artifacts to [storage](#storage). Buckets is backed by the same Xet storage system for faster transfers and lower costs.

This is useful for working environments and keeping the entire ML training loop in one place without leaving the Hub.
