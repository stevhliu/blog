...
Building the model on the meta device doesn't consume any memory. The meta device only holds the data associated with tensors, like shape and dtype. It creates a skeleton of the model.

From the meta tensors' shape and dtype, Transformers computes how many bytes each parameter will occupy once it's loaded. If a device map is being inferred for automatic placement, sharding, CPU/disk offload, or multi-device loading, those sizes help assign parameters to devices with enough available memory.
...
When a safetensors file is opened, `get_slice(key)` returns a lightweight slice object for that tensor. The slice exposes the shape and dtype metadata without materializing the tensor bytes.
...
Transformers' [dynamic weight loading system](https://huggingface.co/docs/transformers/main/weightconverter) is able to load checkpoints that don't match the expected model format at runtime. There are two methods available for aligning the formats.

- `WeightRenaming.convert()` maps checkpoint source tensors to the expected model key by renaming it if it's different.
- `WeightConverter.convert()` performs a series of operations (for example, concatenating q, k, v into a single tensor) that restructures the weights from their checkpoint layout to the model layout.
...
If the sync path loaded tensors immediately instead, every matched weight becomes a real `torch.Tensor` during collection and stay in `collected_tensors` until `convert()` processes them. Returning a callable avoids buildup by deferring each read until the conversion step is ready to consume it.
...
Two parameter names, like `lm_head` and `embed_tokens`, may point to the same underlying tensor. This is known as weight tying. The model uses one shared embedding matrix to read input tokens and to project hidden states back into vocabulary logits. During training, the model updates one shared embedding instead of learning two separate matrices. A useful side effect is that only one tensor allocation is needed for both parameter names.
